...
% Plots self-sufficiency versus number of smart nodes with a smooth trend line.
    ...
    % Protezione contro dati vuoti
    if isempty(nSmart)
        fprintf('  [SKIP] Grafico topologico annullato: nessun dato valido trovato.\n');
        return;
    end
    ...
    % --- FIX ANTIPROIETTILE PER IL POLYFIT ---
    % Traccia la curva solo se ci sono almeno 2 valori di X distinti
    if numel(unique(nSmart)) > 1
        p    = polyfit(nSmart, SS, min(numel(unique(nSmart))-1, 2));
        xfit = linspace(min(nSmart), max(nSmart), 100);
        yfit = polyval(p, xfit);
        plot(xfit, yfit, 'r--', 'LineWidth', 1.5);
    else
        fprintf('  [INFO] Trendline saltata: gli scenari hanno tutti lo stesso numero di nodi (%d).\n', nSmart(1));
    end
